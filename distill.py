...
from utils.loss import compute_loss,compute_fresh_distill_loss
from utils.plots import plot_images, plot_labels, plot_results, plot_evolution
from utils.torch_utils import ModelEMA, select_device, intersect_dicts, torch_distributed_zero_first
from distill_tools.new_head_dist import nh_soft_loss
...
wandb = None # not using wandb

def train(hyp, opt, device, tb_writer=None, wandb=None):
    logger.info(f'Hyperparameters {hyp}')
    save_dir, epochs, batch_size, total_batch_size, teacher_weights, student_weights,rank = \
    ...
    attention_weight, global_weight = \
        opt.attention_weight, opt.global_weight
    ...
    if pretrained:
        ...
        student = Darknet(opt.student_cfg, head_distillation=head_distillation, \
            neck_distillation=neck_distillation, \
                backbone_distillation=backbone_distillation, \
                    neck_feature=student_neck_feature, \
                        backbone_feature=student_backbone_feature).to(device)  # create
        state_dict = {k: v for k, v in ckpt['model'].items() if student.state_dict()[k].numel() == v.numel()}
        student.load_state_dict(state_dict, strict=False)
        print('Student Transferred %g/%g items from %s' % (len(state_dict), len(student.state_dict()), student_weights))  # report
    else:
        student = Darknet(opt.student_cfg, head_distillation=head_distillation, \
            neck_distillation=neck_distillation, \
                backbone_distillation=backbone_distillation, \
                    neck_feature=student_neck_feature, \
                        backbone_feature=student_backbone_feature).to(device) # create student model

    # Create teacher
    teacher = Darknet(opt.teacher_cfg, head_distillation=head_distillation, \
            neck_distillation=neck_distillation, \
                backbone_distillation=backbone_distillation, \
                    neck_feature=teacher_neck_feature, \
                        backbone_feature=teacher_backbone_feature).to(device)
    t_ckpt = torch.load(teacher_weights, map_location=device)  # load teacher checkpoint
    teacher_state_dict = {k: v for k, v in t_ckpt['model'].items() if teacher.state_dict()[k].numel() == v.numel()}
    teacher.load_state_dict(teacher_state_dict, strict=False)
    print('Teacher Transferred %g/%g items from %s' % (len(teacher_state_dict), len(teacher.state_dict()), teacher_weights))  # report
    # TODO 禁用教师梯度
    no_f = True
    for md in teacher.module_list:
        if no_f:
            no_f = False
            continue
        for l in md.parameters():
    ...
    for k, v in dict(student.named_parameters()).items():
        if '.bias' in k:
            pg2.append(v)  # biases
    ...
    # DDP mode
    if cuda and rank != -1:
        mstudent = DDP(student, device_ids=[opt.local_rank], output_device=opt.local_rank)
    ...
    student.class_weights = labels_to_class_weights(dataset.labels, nc).to(device)  # attach class weights
    student.names = names
    ...
    distill_weight = lambda e: ((epochs - e)/epochs)**2

    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        student.train()
        teacher.eval() # TODO 修改文训练模式

        fresh_distill_loss = 0.0
        neck_loss = 0.0
        head_loss = 0.0
        backbone_loss = 0.0
        ...
        if opt.image_weights:
            # Generate indices
            if rank in [-1, 0]:
                cw = student.class_weights.cpu().numpy() * (1 - maps) ** 2  # class weights
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                if teacher_head_out[0].isinf().any() or teacher_head_out[1].isinf().any() or teacher_head_out[2].isinf().any() or teacher_head_out[3].isinf().any():
                    input("INF")
                if student_head_out[0].isinf().any() or student_head_out[1].isinf().any() or student_head_out[2].isinf().any() or student_head_out[3].isinf().any():
                    input("INF")

                loss = 0.0
                o_loss = 0.0
                o_loss, loss_items = compute_loss(student_head_out, targets.to(device), student)  # loss scaled by batch_size
                if opt.origin_loss:
                    loss += o_loss
                    if rank != -1:
                        loss *= opt.world_size  # gradient averaged between devices in DDP mode

                if opt.only_new_head_dist:
                    opt.head_distillation = False
                    head_distillation = False
                    opt.new_head_dist = True

                dh_loss = 0.0
                if opt.new_head_dist:
                    dh_loss, dh_loos_item = compute_fresh_distill_loss(teacher_head_out, student_head_out, targets.to(device), student)
                    # dh_loss = nh_soft_loss(teacher_head_out, student_head_out)
                    fresh_distill_loss += dh_loss
                    loss += opt.new_head_dist_weight * dh_loss * distill_weight

                distillating_loss = 0.0
                if head_distillation:
                    student_head_out = [s.permute(0,1,4,2,3).contiguous().view(s.shape[0], -1,s.shape[2], s.shape[2]) \
                        for s in student_head_out]
                    teacher_head_out = [s.permute(0,1,4,2,3).contiguous().view(s.shape[0], -1,s.shape[2], s.shape[2]) \
                        for s in teacher_head_out]
                distillating_loss, h_loss, n_loss, b_loss = distillation_loss(targets.to(device), \
                    head_distillation=head_distillation, \
                    neck_distillation=neck_distillation, \
                    backbone_distillation=backbone_distillation, \
                    student_head_feature=student_head_out, \
                    student_neck_feature=student_neck_out, \
                    student_backbone_feature=student_backbone_out, \
                    teacher_head_feature=teacher_head_out, \
                ...
                head_loss += h_loss
                neck_loss += n_loss
                backbone_loss += b_loss
                loss += opt.distillation_weight * distillating_loss * distill_weight
            # with open('dist.log', 'a') as fff:
            #     fff.write('total loss: '+ str(loss)+ ' dh_loss '+ str(dh_loss)+ ' dist_loss '+ str(distillating_loss)+'\n')
            # Backward
        ...
        # TODO TensorBoard
        tb_writer.add_scalar('fresh_distill_loos', fresh_distill_loss, global_step=epoch)
        tb_writer.add_scalar('head_distill_loos', head_loss, global_step=epoch)
        tb_writer.add_scalar('neck_distill_loos', neck_loss, global_step=epoch)
        tb_writer.add_scalar('backbone_distill_loos', backbone_loss, global_step=epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--teacher_weights', type=str, default='yolor_p6.pt', help='initial teacher weights path')
    ...
    parser.add_argument('--head_distillation', action='store_true', help='do head distillating')
    parser.add_argument('--neck_distillation', action='store_true', help='do neck distillating')
    parser.add_argument('--backbone_distillation', action='store_true', help='do backbone distillating')
    ...
    parser.add_argument('--target_weight', type=float, default=0.5, help='tasrget weight')
    ...
    parser.add_argument('--new_head_dist', action='store_true', help='use fresh head distillation')
    parser.add_argument('--new_head_dist_weight', type=float, default=1.0, help='use fresh head distillation weights')
    parser.add_argument('--only_new_head_dist', action='store_true', help='only use fresh head distillation')
    parser.add_argument('--origin_loss', action='store_true', help='only use fresh head distillation')
    parser.add_argument('--deweight_dist', action='store_true', help='distllation loss weight lower as opech')
