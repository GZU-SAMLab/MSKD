...
from utils.gen_bmask import gen_binary_scale_mask
...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, is_distill=False):
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, rank=-1, is_distill=False):
        self.img_size = img_size
    ...
    def __getitem__(self, index):
        ...
        # grn binary and scale mask
        if self.is_distill:
            bmask, smask = gen_binary_scale_mask(img.shape, labels[:,1:].astype(np.uint))
